...
def _extract_text_by_layout(file):
    ...
    for i, page in enumerate(pdf):  # Iterate through each page
        ...
        sorted_blocks = sorted(
            blocks, key=lambda b: (round(b[1] / 20), b[0])
        )  # Sort blocks top-to-bottom, left-to-right for better logical reading order
        ...
        if text:
            ...
            )  # Store page as a LangChain Document with metadata
